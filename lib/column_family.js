...
//  ttype.CfDef.call(this, definition);
   ...
   */
  function columnParent(column) {
    var args = {column_family: self.name};
    if(self.isSuper && column) {
      args.super_column = self.columnMarshaller.serialize(column);
    }
   ...
   */
  function columnPath(column, subcolumn) {
    ...
    if(self.isSuper) {
      if(column) args.column = self.columnMarshaller.serialize(column); 
      if(subcolumn) args.subcolumn = self.subcolumnMarshaller.serialize(subcolumn);
    }
    else {
      if(column) args.column = self.columnMarshaller.serialize(column);
    }
  ...
  this.remove = function() {
    ...
    var marshalledKey = this.keyMarshaller.serialize(args.key).toString('binary'),
        path = columnPath(args.column, args.subcolumn),
