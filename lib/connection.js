...
Connection.prototype.cql = function(cmd, args, options, callback){
  ...
  // use cql3 execution if version is set
  if (this.cqlVersion === '3.0.0') {
    if(options.gzip === true){
      zlib.deflate(cql, function(err, cqlz){
        self.execute('execute_cql3_query', cqlz, ttype.Compression.GZIP, this.consistencylevel, onReturn);
      });
    } else {
      self.execute('execute_cql3_query', cql, ttype.Compression.NONE, this.consistencylevel, onReturn);
    }
  }
