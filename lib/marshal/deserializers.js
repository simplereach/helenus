...
/**
 * Decode a Decimal Fixed Point Type
 * @param {String} val The binary string to decode
 * @returns {Number} The number value decoded from the binary string
 */
Deserializers.decodeDecimal = function(val){
  if(val === null || val === undefined){
    return null;
  }

  var buf = new Buffer(val, 'binary');
  var scale = Math.pow(10, buf.readInt32BE(0));
  var value = Deserializers.decodeVarInt(buf.slice(4));
  return value / scale;
};
...
/**
 * Decodes a InetAddress Object
 * @param {String} val The binary string to decode
 * @returns {InetAddress} The inet address value decoded from the binary string
 */
Deserializers.decodeInetAddress = function(val){
    if(val === null || val === undefined){
        return null;
    }
